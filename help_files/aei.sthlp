...
{synopt :{opth tol:erance(aei##options:tolerance)}} tolerance level in termination criterion 10^-{it:n}; default is {bf:tolerance(12)}.{p_end}
...
{synopt :{opth ax:iom(aei##options:axiom)}}  specifies which axiom the user would like to use in testing the data for consistency. The default is {bf: axiom(eGARP)}. In total, there are six axioms that can be tested: eGARP, eWARP, eWGARP, eSARP, eHARP and eCM.{p_end}

{synopt :{opth tol:erance(aei##options:tolerance)}} sets the tolerance level in the termination criterion 10^-{it:n} by specifying the integer number {it: n}.
...
{synopt :{opth suppress:(aei##options:suppress)}} specifies that the user does not want the results displayed in a table. The default is that {bf: suppress} is {it: not} specified. Whether or not this option is specified, the command results are retrievable from {bf: return list}.{p_end}
